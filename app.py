...
mongo=MongoClient(MONGODB_URI,serverSelectionTimeoutMS=8000) if MONGODB_URI else None; db=mongo['ai_website_chatbot'] if mongo is not None else None
bots=db['bots'] if db is not None else None; chunks=db['chunks'] if db is not None else None; jobs=db['jobs'] if db is not None else None
...
def set_job(j,**kw):
    if jobs is not None:kw['updated_at']=now();jobs.update_one({'_id':j},{'$set':kw},upsert=True)
